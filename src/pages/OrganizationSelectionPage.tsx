...
export function OrganizationSelectionPage() {
  ...
  useEffect(() => {
    ...
    let isMounted = true;
    setIsLoading(true);
    fetchPublicOrganizations(decodedCity)
      .then((data) => {
        if (isMounted) {
  ...
  const handleOrganizationSelect = async (org: PublicOrganization) => {
    try {
      const branches = await fetchPublicBranches(org.id);
      ...
      navigate(`/student/branch-selection?org=${org.id}&orgName=${encodeURIComponent(org.name)}`);
    } catch {
      alert('Не удалось получить список филиалов');
    }
  };
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          {org.branchCount} {org.branchCount === 1 ? 'филиал' : org.branchCount < 5 ? 'филиала' : 'филиалов'}
                        </p>
                      </div>
                    </div>
                  </Card>
